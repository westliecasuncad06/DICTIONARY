...
// Database connection
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_word'])) {
        ...
        $sql = "INSERT INTO english (word, wordtype, definition, synonyms, antonyms) VALUES ('$word', '$wordtype', '$definition', '$synonyms', '$antonyms') ON DUPLICATE KEY UPDATE wordtype='$wordtype', definition='$definition', synonyms='$synonyms', antonyms='$antonyms'";
        $conn->query($sql);
        ...
        <header class="header">
            <div class="container">
                ...
                <div class="d-flex justify-content-between align-items-center" style="position: relative;">
                    ...
                    <a href="logout.php" class="btn btn-outline-light" style="position: absolute; top: 20px; right: 20px;"><i class="fas fa-sign-out-alt"></i> Logout</a>
                </div>
            </div>
        </header>
    ...
    $sql = "SELECT word, wordtype, definition, synonyms, antonyms FROM english WHERE word = '$keyword' ORDER BY word LIMIT 50";
                    ...
                    if ($result->num_rows > 0) {
                        while($row = $result->fetch_assoc()) {
                            ...
                            echo '<div>';
                            // Use word as id substitute since no id column
                            $wordId = htmlspecialchars($row['word']);
                            echo '<button class="btn btn-edit me-2" data-bs-toggle="modal" data-bs-target="#editWordModal" data-id="' . $wordId . '" data-word="' . htmlspecialchars($row['word']) . '" data-wordtype="' . htmlspecialchars($row['wordtype']) . '" data-definition="' . htmlspecialchars($row['definition']) . '" data-synonyms="' . htmlspecialchars($row['synonyms']) . '" data-antonyms="' . htmlspecialchars($row['antonyms']) . '"><i class="fas fa-edit"></i> Edit</button>';
                            echo '<button class="btn btn-delete" data-bs-toggle="modal" data-bs-target="#deleteWordModal" data-id="' . $wordId . '" data-word="' . htmlspecialchars($row['word']) . '"><i class="fas fa-trash"></i> Delete</button>';
                            echo '</div>';
                            echo '</div>';
        ...
        <footer class="footer">
            <div class="container">
                <p>&copy; 2024 Admin Dictionary. Powered by comprehensive word data.</p>
            </div>
        </footer>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                editModal.addEventListener('show.bs.modal', function (event) {
                    ...
                    const wordtype = button.getAttribute('data-wordtype');
                    const definition = button.getAttribute('data-definition');
                    const synonyms = button.getAttribute('data-synonyms');
                    const antonyms = button.getAttribute('data-antonyms');

                    const modal = this;
                    modal.querySelector('#edit_id').value = id;
                    modal.querySelector('#edit_word').value = word;
                    modal.querySelector('#edit_wordtype').value = wordtype;
                    modal.querySelector('#edit_definition').value = definition;
                    modal.querySelector('#edit_synonyms').value = synonyms;
                    modal.querySelector('#edit_antonyms').value = antonyms;
                });
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form method="POST">
                            <div class="mb-3">
                                <label for="add_word" class="form-label">Word</label>
                                <input type="text" class="form-control" id="add_word" name="word" required>
                            </div>
                            <div class="mb-3">
                                <label for="add_wordtype" class="form-label">Word Type</label>
                                <input type="text" class="form-control" id="add_wordtype" name="wordtype">
                            </div>
                            <div class="mb-3">
                                <label for="add_definition" class="form-label">Definition</label>
                                <textarea class="form-control" id="add_definition" name="definition" rows="3" required></textarea>
                            </div>
                            <div class="mb-3">
                                <label for="add_synonyms" class="form-label">Synonyms (comma separated)</label>
                                <textarea class="form-control" id="add_synonyms" name="synonyms" rows="2"></textarea>
                            </div>
                            <div class="mb-3">
                                <label for="add_antonyms" class="form-label">Antonyms (comma separated)</label>
                                <textarea class="form-control" id="add_antonyms" name="antonyms" rows="2"></textarea>
                            </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form method="POST">
                            ...
                            <div class="mb-3">
                                <label for="edit_wordtype" class="form-label">Word Type</label>
                                <input type="text" class="form-control" id="edit_wordtype" name="wordtype">
                            </div>
                            <div class="mb-3">
                                <label for="edit_definition" class="form-label">Definition</label>
                                <textarea class="form-control" id="edit_definition" name="definition" rows="3" required></textarea>
                            </div>
                            <div class="mb-3">
                                <label for="edit_synonyms" class="form-label">Synonyms (comma separated)</label>
                                <textarea class="form-control" id="edit_synonyms" name="synonyms" rows="2"></textarea>
                            </div>
                            <div class="mb-3">
                                <label for="edit_antonyms" class="form-label">Antonyms (comma separated)</label>
                                <textarea class="form-control" id="edit_antonyms" name="antonyms" rows="2"></textarea>
                            </div>
                            <button type="submit" name="edit_word" class="btn btn-primary">Update Word</button>
                        </form>
                    </div>
                </div>
            </div>
        ...
        <div class="modal fade" id="deleteWordModal" tabindex="-1" aria-labelledby="deleteWordModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p>Are you sure you want to delete the word "<span id="delete_word_text"></span>"?</p>
                        <form method="POST">
                            <input type="hidden" id="delete_id" name="id">
                            <button type="submit" name="delete_word" class="btn btn-danger">Delete</button>
